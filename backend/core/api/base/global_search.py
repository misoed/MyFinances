...
def global_search_endpoint(request):
    ...
    def get_permitted_clients(request):
        ...
        elif request.team.is_owner(request.user) or "clients:read" in TeamMemberPermission.objects.filter(
            team=request.team, user=request.user).first().scopes:
            ...
            if exact_invoice:
                resources["invoice"].append({
                    ...
                    "details": {
                        ...
                        "Total Amount": f"{exact_invoice.get_total_price()} {exact_invoice.currency}" if exact_invoice.get_total_price() else "N/A",
            ...
            for invoice in exact_invoice:
                resources["invoice"].append({
                    ...
                    "details": {
                        ...
                        "Company" if matched_filter == "client_name" else "Client": invoice.client_company if matched_filter == "client_name" else invoice.client_name,
                    }
                })
